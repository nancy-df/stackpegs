...
4. When the canvas holds two or more tools, after an 800 ms pause a request is sent to the server. While it runs, a large translucent "Mapping integrations..." message with a spinner pulses in the center of the canvas (like a watermark) so it is obvious the app is working. It does not block clicks, is announced to screen readers, and disappears when the result arrives. The Regenerate button also shows "Working...".
