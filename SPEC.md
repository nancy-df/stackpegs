...
4. When the canvas holds two or more tools, after an 800 ms pause a request is sent to the server. A "Mapping integrations..." indicator shows at the bottom of the canvas while it runs.
...
7. Remove a tool with its hover "x" (or the Delete key in the Free view), or "Clear" the canvas. The toolbar has **All connections** (Stack view) or **Auto-layout** (Free view), Download HTML, and Clear. The word "HTML" is dropped from the Download button when the canvas is narrow.
...
- **Quiet connections.** By default every connection is a faint gray line with no arrowhead or label, so the strips read as a stack. Hovering or focusing a tool, or selecting a tool or connection, lights up that tool's connections in their type color with arrowheads and labels and dims everything else. **All connections** in the toolbar shows every connection in color with labels. Clicking a line or label selects it and opens its detail.
...
- **A legend** below the strips lists the connection types in use.
...
- Built on React Flow. Pan and zoom. Nodes are draggable; one node per tool (dropping a tool that is already there just selects it). This is the original layout, with the Auto-layout button.
